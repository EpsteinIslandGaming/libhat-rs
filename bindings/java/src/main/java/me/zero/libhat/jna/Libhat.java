...
/**
 * JNA bindings for the libhat native library.
 * Build the shared library with:
 * <pre>cargo build --release --lib</pre>
 * Output: {@code target/release/libhat.so} (Linux),
 *         {@code target/release/hat.dll} (Windows),
 *         {@code target/release/libhat.dylib} (macOS).
 * <p>
 * Place the shared library on {@code java.library.path} or in the working directory.
 */
public interface Libhat extends Library {

    Libhat INSTANCE = Native.load("hat", Libhat.class);
