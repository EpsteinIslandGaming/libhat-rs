...
public final class Hat {
    ...
    public static @NotNull Signature parseSignature(@NotNull final String signature) {
        ...
        final int statusCode = Libhat.INSTANCE.libhat_parse_signature(signature, handle);
        final Status status = Status.fromCode(statusCode);
        if (status != Status.SUCCESS) {
            throw new RuntimeException("libhat error " + status);
        }
    ...
    public static @NotNull Signature createSignature(final byte @NotNull[] bytes, final byte @NotNull[] mask) {
        ...
        final Pointer[] handle = new Pointer[1];
        final int statusCode = Libhat.INSTANCE.libhat_create_signature(bytes, mask, bytes.length, handle);
        final Status status = Status.fromCode(statusCode);
        if (status != Status.SUCCESS) {
            throw new RuntimeException("libhat error " + status);
        }
